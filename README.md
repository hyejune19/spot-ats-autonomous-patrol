...
**ATS**는 이 프로젝트에서 Spot 상부에 결합한 **2축 회전 장치**를 가리킨다. 좌우·상하 방향을 조절하는 기구를 보행 제어와 분리한다. 명칭의 영어 확장은 원본 자료에서 확인되지 않아 임의로 정의하지 않는다.
...
- [ATS 기반 원본 프로젝트](https://github.com/engiengi/SpotATS_ws)

**USD·가중치·강의 PDF·인증정보는 포함하지 않는다.** 원본 ATS의 라이선스 조건에 따라 수정된 외부 소스는 재배포하지 않고 로컬 의존성으로 연결한다. 따라서 실행에는 별도의 로컬 모델·학습 정책·작업 공간이 필요하다.
